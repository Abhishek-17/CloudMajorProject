		...
		<div class="wrapper">
			<?php include 'sidebar.php';
				...
				<div class="article">
                                    ...
                                    if(isset($_GET['con_id']))
                                    {
                                        if(isset($_POST['con_name']) && isset($_POST['con_manager_id']) && isset($_POST['con_deadline']))
                                        {
                                            ...
                                            else if($_POST['con_name']!="" && $_POST['con_manager_id']!="" && $_POST['con_deadline']!="")
                                            {
                                                ...
                                                echo $org;
                                                //$sql='update contract set con_name="'.$_POST['con_name'].'" , con_manager_id='.$_POST['con_manager_id'].' , con_deadline='.$_POST['con_deadline'].' , org_id="'.$org.'" where id='.$_GET['con_id'].'';
                                                $sql='delete from contract where id='.$_GET['con_id'];
                                                $result=mysqli_query($con,$sql);
                                                ...
                                                insertintodb("contract", array($_GET['con_id'],$_POST['con_name'],$_POST['con_manager_id'],$_POST['con_deadline'],$org), 1);
                                            ...
                                            {
                                                header('Location: edit_contract.php?message="Some field was not filled"');
                                        ...
                                        {
                                            $sql='select con_name,con_manager_id,con_deadline,org_id from contract where id='.$_GET['con_id'];
                                            $result=mysqli_query($con,$sql);
                                            $row=  mysqli_fetch_array($result);
                                            echo '
                                            <h1><center><font face="Arial" color="#FF2222">ADD CONTRACT</font></center></h1>
                                            <form action="edit_contract.php?con_id='.$_GET['con_id'].'" id="login-form1" method="post">
                                                <center><table><tbody>
                                                <tr>
                                                <td>CONTRACT NAME  </td><td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="con_name" value="'.$row['con_name'].'"/></td>
                                                </tr>
                                                <tr>
                                                    <td>&nbsp;</td>
                                                    <td>&nbsp;</td>
                                                ...
                                                <tr>
                                                <td>CONTRACT DEADLINE </td><td>&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" name="con_deadline" value="'.$row['con_deadline'].'"/></td>
                                                </tr>
                                                <tr>
                                                    <td>&nbsp;</td>
                                                    <td>&nbsp;</td>
                                                ...
                                                <td>&nbsp;&nbsp;&nbsp;&nbsp;';
                                                $sql="select id,org_name from organization";
                                                $org=  unserialize($row['org_id']);
                                                //$result=selectfromdb("organization",array("id",'org_name'),"org_name!='root'");
                                                $result2=  mysqli_query($con, $sql);
                                                while($row2 = mysqli_fetch_array($result2))
                                                {
                                                   if(in_array($row2['id'], $org))
                                                   {
                                                        echo '<input type="checkbox" name="'.$row2['org_name'].'"checked >'.$row2['org_name'].'</option>';
                                                   }
                                                   else
                                                   {
                                                        echo '<input type="checkbox" name="'.$row2['org_name'].'">'.$row2['org_name'].'</option>';
                                                   }
                                                }
                                                echo '
                                                </td>
                                                ...
                                                <tr>
                                                <td></td>
                                                <td>&nbsp;&nbsp;&nbsp;&nbsp;<div class="wrapper"><input type="submit" value="Update" class="submit fleft" /></div></td>
                                                </tr>
                                                </tbody></table></center>
                                                </form>';
                                        }
                                    ...
                                    {
                                        $sql="select id,con_name from contract";
                                        $result=mysqli_query($con,$sql);
                                        if(mysqli_num_rows($result))
                                        {
                                            while($row=  mysqli_fetch_array($result))
                                            {
                                                echo '<center><a href="edit_contract.php?con_id='.$row['id'].'" >EDIT&nbsp;&nbsp;&nbsp;"'.$row['con_name'].'"</a></center><br>';
                                            }
                                        }
                                        else
                                        {
                                            echo '<center><h3>No contract exists</h3></center>';
                                        }
                                    }
                                    ?>
                               </div>
