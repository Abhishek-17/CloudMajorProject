...
$parentworkflow = array((string) $xmll['name'], "ist work flow", date("Y/m/d"), date("Y/m/d"), (string) $xmll['user']);
insertintodb("parent_workflow", $parentworkflow, 0);
$pwid = getmaxidfromdb("parent_workflow");

foreach ($xmll->workflow as $xml) {
    $workflow = array($pwid, (string) $xml['name'], "ist work flow", date("Y/m/d"), date("Y/m/d"), $user_id, 0,serialize(array()));
...
//exit(0);
    ...
    foreach ($xml->node as $node) {
        ...
        foreach ($node->input as $input) {
            ...
            foreach ($input->value as $value) {
                ...
                //--------------------------------------------
                    ...
                    foreach ($value->condition as $condition){
                        ...
                        $fl=explode(",",$v[1]);
                        if(count($fl)==2)array_push($fval,$fl);
                        array_push($conditions,$v);
                    }
                   // echo $value['table']."<br>"; echo $value['field']."<br>";exit(0);
            ...
            // echo "<h1>".count($record)."</h1>";
            if(!isset($node['substitute']))insertintodb("inputs", $record, 0);
        ...
        foreach($node->mustbedeactive as $lst){
            foreach($lst->id as $id)
                if(!in_array($id,$deactivatedd))array_push($deactivatedd,(string)$id);
        }
//   / print_r($deactivatedd);echo " ".$node['id']."<br>";
        $record = array($wid, $node['id'], $node['type'], serialize($innodes), serialize($done_innodes), serialize($out), serialize($functions), serialize($branch), serialize($fallback), serialize($path), $activated, $starttime, $time, 1, $cost,  serialize($fval),serialize($deactivatedd));
         if(isset($node['substitute'])){
          //   echo "came<br>";
           $result22=  selectfromdb("function_workflow",array("name","id"),"name='".$node['substitute']."'");
            ...
            else{insertintodb("transition", $record, 1);}
           // echo "returned<br>";
        }
        else insertintodb("transition", $record, 1);
    }
...
//exit(0);
logg("added workflow wid=".$wid);
header('Location: upload_file.php?message="Workflow added successfully"');
//  exit(0);
?>