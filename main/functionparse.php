...
//echo  (string)$xml->node[0]['id'];
    $mapping=array();
    insertintodb("function_workflow", $workflow, 0);
  //  exit(0);

//exit(0);
    $wid = getmaxidfromdb("function_workflow");
    echo $wid;//exit(0);
    foreach ($xml->node as $node) {
        ...
        foreach($node->mustbedeactive as $lst){
            foreach($lst->id as $id)
                if(!in_array($id,$deactivated))array_push($deactivated,(string)$id);
        }
...
//exit(0);
logg("added function_workflow wid=".$wid);
