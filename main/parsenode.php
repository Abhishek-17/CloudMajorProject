...
// and at least an element /[root]/title.
    ...
    foreach ($xml->node as $node) {
        ...
        $delete = "delete from inputs where workflow_id=" . $wid . " and transition_id=" . $_POST['nodeid'];
        if (!mysqli_query($con, $delete)) {
            die('Error :' . mysqli_error($con));
        }
        $result = selectfromdb("transition", array("activated", "start_time", "time_limit", "doer"), "workflow_id=" . $wid . " and id=" . $_POST['nodeid']);
        $myrow = mysqli_fetch_array($result);
        $delete = "delete from transition where workflow_id=" . $wid . " and id=" . $_POST['nodeid'];
        if (!mysqli_query($con, $delete)) {
            die('Error :' . mysqli_error($con));
        }
      $out = array();
        ...
        $record = array($wid, $node['id'], $node['type'], serialize($innodes), serialize($done_innodes), serialize($out), serialize($functions), serialize($branch), serialize($fallback), serialize($path), $activated, $starttime, $time, 1, $cost,  serialize($fval),serialize($deactivated));
         if(isset($node['substitute'])&&0){//not allowed
          //   echo "came<br>";
    ...
    $result=selectfromdb("workflow",array("mapping"),"id=".$wid);
    $row=  mysqli_fetch_array($result);
    $mapping=unserialize($row['mapping']);
    
    if(count($mapping))remapworkflow($wid,$mapping);//
    logg("modified node wid=".$_POST['wid']." nodeid=".$_POST['nodeid']);
